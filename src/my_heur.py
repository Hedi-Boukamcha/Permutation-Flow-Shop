...
def heuristic_due_date_pfsp(instance,
    ...
    for it in range(max_iter):
        ...
        partial, removed = destruction_critical(current_seq, pt, due_dates, weights, objective, k=k, rng=rng)

        # 3) reconstruction orientée impact
        new_seq = reconstruction_impact(partial, removed, pt, due_dates, weights, objective)
        ...
        if no_improve >= 15:
            ...
            partial, removed = destruction_critical(current_seq, pt, due_dates, weights, objective, k=k_big, rng=rng)
            current_seq = reconstruction_impact(partial, removed, pt, due_dates, weights, objective)
